...
import { plotManagementService, PlotWithOrderStatus } from './plotsManage';
...
class PlotService {
  ...
  // Always check backend API health
    ...
    try {
      ...
      if (!isHealthy) {
  if (import.meta.env.MODE === 'development') console.warn('[PlotService] API health check failed:', response.status);
      }
      
      return isHealthy;
    } catch (error) {
  if (import.meta.env.MODE === 'development') console.error('[PlotService] Health check error:', error);
      isHealthy = false;
      lastHealthCheck = now;
      return false;
    }
  ...
  private async fetchWithErrorHandling(url: string, options?: RequestInit): Promise<Response> {
    ...
    try {
  if (import.meta.env.MODE === 'development') console.log(`[PlotService] Making API request to: ${url}`);
  ...
  if (import.meta.env.MODE === 'development') console.error(`[PlotService] ❌ API responded with error:`, {
  ...
  if (import.meta.env.MODE === 'development') console.error('[PlotService] Network/Request error:', error);
  ...
  async getAllPlots(): Promise<PlotWithOrderStatus[]> {
  // Always use backend API
    
    try {
  if (import.meta.env.MODE === 'development') console.log('[PlotService] Fetching all plots from API...');
      const response = await this.fetchWithErrorHandling(`${API_BASE}/api/plots`);
      const data = await response.json();
      
  if (import.meta.env.MODE === 'development') console.log('[PlotService] Raw API response type:', typeof data, 'features count:', data?.features?.length);
  ...
  if (import.meta.env.MODE === 'development') console.warn('[PlotService] No plots found in database');
      ...
      const plots: Plot[] = data.features.map((feature: any, index: number) => {
        try {
          if (!feature.properties || !feature.geometry) {
            if (import.meta.env.MODE === 'development') console.warn(`[PlotService] Feature ${index} missing properties or geometry`);
            return null;
          }
          ...
          if (!props.id || !props.plot_code) {
            if (import.meta.env.MODE === 'development') console.warn(`[PlotService] Feature ${index} missing required properties (id or plot_code)`);
            return null;
          }
          
          // Validate geometry
          if (!feature.geometry.coordinates || feature.geometry.coordinates.length === 0) {
            if (import.meta.env.MODE === 'development') console.warn(`[PlotService] Feature ${index} has invalid geometry`);
            return null;
          }
          ...
          if (import.meta.env.MODE === 'development') console.error(`[PlotService] Error processing feature ${index}:`, error);
          return null;
        }
      }).filter(Boolean) as Plot[]; // Remove null entries safely
      
  if (import.meta.env.MODE === 'development') console.log(`[PlotService] ✅ Successfully processed ${plots.length} plots from ${data.features.length} features`);
      
      if (plots.length === 0) {
  if (import.meta.env.MODE === 'development') console.warn('[PlotService] ⚠️ No valid plots found in API response');
        return [];
      }
      
      // Enrich plots with order status information
      const enrichedPlots = await plotManagementService.enrichPlotsWithOrderStatus(plots);
      
  if (import.meta.env.MODE === 'development') console.log(`[PlotService] ✅ Enriched ${enrichedPlots.length} plots with order status`);
      
      return enrichedPlots;
      
    } catch (error) {
  if (import.meta.env.MODE === 'development') console.error('[PlotService] ❌ Error fetching plots:', error);
  ...
  // Always use backend API
    ...
    try {
  if (import.meta.env.MODE === 'development') console.log(`Fetching plot ${plotId}...`);
  ...
  if (import.meta.env.MODE === 'development') console.error(`Error fetching plot ${plotId}:`, error);
      return null;
    }
  }
  ...
  // Always use backend API
    ...
    try {
  if (import.meta.env.MODE === 'development') console.log(`[PlotService] Creating order for plot ${plotId}`);
  ...
  if (import.meta.env.MODE === 'development') console.log('[PlotService] ✅ Order created successfully:', order.id);
      
      return order;
      
    } catch (error) {
  if (import.meta.env.MODE === 'development') console.error(`[PlotService] ❌ Error creating order for plot ${plotId}:`, error);
  ...
  if (import.meta.env.MODE === 'development') console.log(`Searching plots with filters:`, filters);
  ...
  if (import.meta.env.MODE === 'development') console.log(`Found ${plots.length} plots matching search criteria`);
      return plots;
      
    } catch (error) {
  if (import.meta.env.MODE === 'development') console.error('Error searching plots:', error);
  ...
  // Always use backend API
    ...
    try {
  if (import.meta.env.MODE === 'development') console.log('Fetching system statistics...');
      const response = await this.fetchWithErrorHandling(`${API_BASE}/api/stats`);
      const stats = await response.json();
      
  if (import.meta.env.MODE === 'development') console.log('System stats:', stats);
      return stats;
      
    } catch (error) {
  if (import.meta.env.MODE === 'development') console.error('Error fetching system stats:', error);
  ...
  if (import.meta.env.MODE === 'development') console.log('Fetching orders...');
      
      const response = await this.fetchWithErrorHandling(url);
      const data = await response.json();
      
  if (import.meta.env.MODE === 'development') console.log(`Fetched ${data.orders?.length || 0} orders`);
      return data;
      
    } catch (error) {
  if (import.meta.env.MODE === 'development') console.error('Error fetching orders:', error);
  ...
  if (import.meta.env.MODE === 'development') console.error('[PlotService] Health check failed:', error);
      return { 
        healthy: false, 
  ...
  subscribeToPlotUpdates(_: (plots: Plot[]) => void): () => void {
  // Always use backend API
